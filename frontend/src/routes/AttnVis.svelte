<script>
    ...
    export async function update_attention_container() {
        ...
        console.log("n_views = ", n_views, typeof n_views);
    ...
    function visualize_multiple_attention(
        ...
        if (selected_heads !== null && selected_heads.length < n_views) {
            while (selected_heads.length < n_views) {
                selected_heads.push([0, 0]);
            }
        }
        ...
        function drawTokens(
            ...
            const attention_matrix =
                attention_mappings.attention_matrices[layer_idx][head_idx];
            ...
            let color_scale = d3
                .scaleSequential()
                .domain([
                    d3.min(attention_distribution),
                    d3.max(attention_distribution),
                ])
                .interpolator(color);
            ...
            tokens.forEach((src_token, i) => {
                let color_rgb =
                    i <= target_idx
                        ? color_scale(attention_distribution[i])
                        : "rgb(255, 255, 255)";
        ...
        // Outer container
        const main_container_div = d3
            .create("div")
            .attr(
                "style",
                "display: flex; flex-wrap: wrap; flex-direction: column; gap: 1em; padding: 1em;"
            );

        for (let view_no = 0; view_no < n_views; view_no++) {
            const view_container_div = main_container_div
                .append("div")
                .attr("id", `view_${view_no}`);
            ...
            const config_div = view_container_div
                .append("div")
                .attr(
                    "style",
                    "display: flex; flex-wrap: wrap; gap: 0.5em; padding: 0.5em;"
                );

            // Layer Select
            const layer_config_div = config_div
                .append("div")
            ...
            const layer_select = layer_config_div
                .append("select")
            ...
            // Head Select
            const head_config_div = config_div
                .append("div")
                .attr("style", "display: flex;");
            head_config_div
                .append("label")
            ...
            const head_select = head_config_div
                .append("select")
                .attr("id", "head-select");
            ...
            const token_container = view_container_div
                .append("div")
        ...
        function update_all_views(target_idx) {
            ...
            for (let view_no = 0; view_no < n_views; view_no++) {
                const layer_selector = d3
                    .select(`#view_${view_no}`)
                    .select("#layer-select");
                const head_selector = d3
                    .select(`#view_${view_no}`)
                    .select("#head-select");
                const token_view = d3
                    .select(`#view_${view_no}`)
                ...
                console.log(
                    view_no,
                    `L: ${layer_selector.node().value}`,
                    `H: ${head_selector.node().value}`
                );

                drawTokens(
                    target_idx,
                    layer_selector.node().value,
                    head_selector.node().value,
                    token_view,
                    colors[view_no]
                );
            }
...
<div bind:this={attn_vis_container}></div>
